...
def tokenize(query):
    stopword = ['the', 'a', 'an', 'in','of','that','then','is','are','was','were','out']
    tokens = query.split()
...
def doc_tf_idf(dataframe,query):
    ...
    doc_frequency = np.log10(60 / (doc_term_value ))
    ...
    for i in range(len(dataframe)):
        ...
        for j,term in enumerate(row_index):
            ...
            if term in query:
                ...
                results[j] = term_frequency * (np.log10(60 / (new_col_value[0] + 1)))
...
for ind_file,similarity in score:
    print("Doc {} query similarty is {}".format(ind_file+1,similarity))